...
// shellcode for `liblv2_patch`.
// compile with powerpc64 clang (trunk) + `-Os`
// because triple of ppu-lv2 doesn't support any kind of optimization
#if 0
...
// static_assert(sizeof(bigint::c) == 8, "");
#define pChar bigint
...
extern "C"
{
int sys_prx_load_module(const void*, void*, void*);
int sys_prx_start_module(int, uint64_t, void*, int*, void*, void*);
void run_prog_main(uint32_t param_1, void* param_2, void* param_3, void* param_4);

#if defined(COMPILE_INLINE)
int enable_trap(void)
{
    return 0;
}
#else
...
#endif
#if defined(COMPILE_INLINE)
void* my_memset(void* x, uint8_t fill, uint32_t size)
{
...
void lv2_preload(uint32_t argc, uint32_t argv, void* param_3, void* param_4)
{
    // auto generated by char to u32. please don't edit this manually.
    // string length (37)
    // /dev_hdd0/game_plugin_bootloader.sprx
    uint32_t path[10];
    path[0] = __builtin_bswap32(0x7665642f);
    path[1] = __builtin_bswap32(0x6464685f);
    path[2] = __builtin_bswap32(0x61672f30);
    path[3] = __builtin_bswap32(0x705f656d);
    path[4] = __builtin_bswap32(0x6967756c);
    path[5] = __builtin_bswap32(0x6f625f6e);
    path[6] = __builtin_bswap32(0x6f6c746f);
    path[7] = __builtin_bswap32(0x72656461);
    path[8] = __builtin_bswap32(0x7270732e);
    path[9] = __builtin_bswap32(0x78);
    // printf(path);
    int prx = sys_prx_load_module(path, 0, 0);
    int ret;
    int res = 0;
    if (prx < 0)
    {
    ...
    a.argc = argc;
    a.argv = argv;
    a.replace_args_size = sizeof(a.replace_args);
    a.replace_args_per_buf_size = sizeof(a.replace_args[0]);
    ret = sys_prx_start_module(prx, 1, &a, &res, 0, 0);
    if (ret < 0)
    {
        if (enable_trap())
        {
...
err:
    run_prog_main(a.argc,
                  a.flag.changed ? (uint32_t*)a.new_argv : (uint32_t*)a.argv,
                  param_3,
                  param_4);
    return;
}
...
static const uint32_t liblv2_patch[] =
{
	0x48000074,
	0x480100c0, // sys_prx_load_module
	0x4800f930, // sys_prx_start_module
	0x4bfff038, // run_prog_main
	0x38600000, // enable to get execptions
	0x4e800020,
	0xf8a1ffe0,
	0x7c651b78,
	0xe861ffe0,
...
#define read_mem(p,a,b,s) process_read_memory(p,(void*)(uint64_t)(a),(void*)(b),(s))
#define write_mem(p,a,b,s) process_write_memory(p,(void*)(uint64_t)(a),(void*)(b),(s),1)

uint8_t proc_load_okay = 0;

static int get_text_info(process_t process, const char* modname, uintptr_t *outBase, uint64_t *outSize);
...
LV2_HOOKED_FUNCTION_PRECALL_SUCCESS_8(int, load_process_hooked, (process_t process, int fd, char *path, int r6, uint64_t r7, uint64_t r8, uint64_t r9, uint64_t r10, uint64_t sp_70))
{
	...
	if ((strstr(path, "/dev_bdvd/PS3_GAME/USRDIR/") || strstr(path, "/dev_hdd0/game/")))
	{
		DPRINTF2("proc_load_okay\n");
		proc_load_okay = 1;
		
		// need this code, otherwise kernel complains about stack corruption in `load_module_hook`
		uintptr_t lv2_base = 0;
		uint64_t lv2_size = 0; // text only!
		get_text_info(process, "liblv2", &lv2_base, &lv2_size);
		// DPRINTF2("lv2 base 0x%lx 0x%lx\n", lv2_base, lv2_base+lv2_size);
	}
	return SUCCEEDED;
}
...
#define MAX_PRX 256
	struct d
	{
		sys_prx_id_t list[MAX_PRX];
		uint32_t unk_list[MAX_PRX];
		uint32_t n, n2;
	} d;
	memset(&d, 0, sizeof(d));
	if (prx_get_module_list(process, d.list, d.unk_list, _countof(d.list), &d.n, &d.n2) == 0)
	{
		for (uint32_t i = 0; i < d.n; i++)
		{
			...
			d2.modinfo.segments_num = 1;

			if (prx_get_module_info(process, d.list[i], &d2.modinfo, NULL, &d2.text_segment) == 0)
			{
				// liblv2
				// DPRINTF("<%d:%d:%s> [%08lX]\n", i, list[i], modinfo.name, text_segment.base);
				if (strstr(d2.modinfo.name, modname) != 0)
				{
					*outBase = d2.text_segment.base;
					*outSize = d2.text_segment.memsz;
					return 1;
				}
			}
		}
...
// masked by ff
static uintptr_t PatternScan(process_t process, const uintptr_t module_base, const uint64_t module_size, const void *signature, const uint64_t signature_size, const uint64_t offset)
{
#define seek_size 256
	...
	for (uintptr_t seek = module_base; seek < ((module_base + module_size) - signature_size); seek += seek_size)
	{
		...
		if (read_mem(process, seek, seek_buf, sizeof(seek_buf)) == 0)
		{
			const uint8_t *scanBytes = seek_buf;
			const uint8_t *patternBytes = signature;
...
int upload_liblv2_code(process_t process)
{
	{
		uintptr_t lv2_base = 0;
		uint64_t lv2_size = 0; // text only!
		...
		{
			uint32_t buf = 0;
			const uintptr_t lv2_max = lv2_base + lv2_size;
			const size_t patch_size = sizeof(liblv2_patch);
			const int ret = read_mem(process, lv2_base, &buf, sizeof(buf));
			if (ret == 0)
			{
				const uint32_t lv2_check = 0x39c00000; // at least for 4.84
				const uintptr_t code_start = (lv2_max - 4) - patch_size;
				DPRINTF2("lv2 read %x check %x ret %d\n", buf, lv2_check, ret);
				DPRINTF2("lv2 base %lx size %lx (max %lx)\n", lv2_base, lv2_size, lv2_max);
				DPRINTF2("code size %lx seeked %lx\n", patch_size, code_start);
				if (buf == lv2_check)
				{
					static const uint32_t run_prog_main[] = {
						0x60000000,
						0x7fa3eb78,
						0x7f84e378,
						0x7fe5fb78,
					...
					static const uint32_t sys_prx_start_module[] = {
						0x38c10070,
						0x38e00000,
						0x39000000,
						0x38800000,
					...
					static const uint32_t sys_prx_load_module[] = {
						0x8062ffff,
						0x38800000,
						0x38a00000,
						0xffffffff,
					...
					static uint32_t patch_copy[_countof(liblv2_patch)] = {0};
					memset(patch_copy, 0, sizeof(patch_copy));

					const uintptr_t main = PatternScan(process, lv2_base, lv2_size, run_prog_main, sizeof(run_prog_main), 20);
					const uintptr_t start = PatternScan(process, lv2_base, lv2_size, sys_prx_start_module, sizeof(sys_prx_start_module), 24);
					const uintptr_t load = PatternScan(process, lv2_base, lv2_size, sys_prx_load_module, sizeof(sys_prx_load_module), 12);
					memcpy(patch_copy, liblv2_patch, sizeof(liblv2_patch));
					enum
					{
						sys_prx_start_module_idx = 1,
						sys_prx_load_module_idx,
						run_prog_main_idx,
						// printf_idx,
					};
					const uintptr_t load_start = code_start + (sys_prx_load_module_idx * 4);
					const uintptr_t start_start = code_start + (sys_prx_start_module_idx * 4);
					const uintptr_t main_start = code_start + (run_prog_main_idx * 4);
					// const uintptr_t printf_start = code_start + (printf_idx * 4);
					uint32_t load_buf, start_buf, main_buf;
					read_mem(process, load, &load_buf, sizeof(load_buf));
					read_mem(process, start, &start_buf, sizeof(start_buf));
					read_mem(process, main, &main_buf, sizeof(main_buf));
					patch_copy[sys_prx_load_module_idx] = MAKE_JUMP_VALUE(load_start, (load + READ_CALL_OFFSET(load_buf)));
					patch_copy[sys_prx_start_module_idx] = MAKE_JUMP_VALUE(start_start, (start + READ_CALL_OFFSET(start_buf)));
					patch_copy[run_prog_main_idx] = MAKE_JUMP_VALUE(main_start, (main + READ_CALL_OFFSET(main_buf)));
					// patch_copy[printf_idx] = MAKE_JUMP_VALUE(printf_start, (lv2_base + 0x0000fc54));
					const uint32_t main_to_code_start = MAKE_CALL_VALUE(main, code_start);
#define PRINT_BRANCH(b, s) // DPRINTF(#b ": from 0x%lx 0x%x to 0x%lx\n", s, b, s + READ_JUMP_OFFSET(b))
					PRINT_BRANCH(main_to_code_start, main);
					PRINT_BRANCH(patch_copy[sys_prx_load_module_idx], load_start);
					PRINT_BRANCH(patch_copy[sys_prx_start_module_idx], start_start);
					PRINT_BRANCH(patch_copy[run_prog_main_idx], main_start);
#undef PRINT_BRANCH
					write_mem(process, main, &main_to_code_start, sizeof(main_to_code_start));
					write_mem(process, code_start, patch_copy, sizeof(patch_copy));
					memset(patch_copy, 0, sizeof(patch_copy));
					return SUCCEEDED;
				}
			}
...
LV2_HOOKED_FUNCTION_POSTCALL_3(int, load_module_hook, (char *path, uint64_t param_2, void *param_3))
{
	if (!proc_load_okay || safe_mode)
	{
		return SUCCEEDED;
	}
	process_t process = get_current_process();
	...
	DPRINTF2("Load module: %s (%s)\n", path, get_process_name(process) + 8);
	if (proc_load_okay && strstr(path, "/dev_flash/sys/external/libsysmodule.sprx"))
	{
		proc_load_okay = 0;
		DPRINTF2("passed check\n");
		upload_liblv2_code(process);
		return SUCCEEDED;
	}
	return SUCCEEDED;
}

static void do_hook_load_module(void)
{
	const uint64_t load = get_syscall_address(_SYS_PRX_LOAD_MODULE);
	hook_function_with_postcall(load, load_module_hook, 3);
}

static void do_unhook_load_module(void)
{
	const uint64_t load = get_syscall_address(_SYS_PRX_LOAD_MODULE);
	unhook_function_with_postcall(load, load_module_hook, 3);
}
