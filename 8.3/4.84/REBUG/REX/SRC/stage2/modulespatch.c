...
static int read_write_mem(process_t process, uint64_t addr, void *buf, int size, int write_)
{
	if (process <= 0)
	{
		return ESRCH;
	...
	else
	{
		if (write_)
		{
			return process_write_memory(process, (void *)addr, buf, size, 1);
		}
		else
		{
			return copy_from_process(process, (void *)addr, buf, size);
		}
	}
	return ESRCH;
}

#define read_mem(p,a,b,s) read_write_mem(p,a,b,s,0)
#define write_mem(p,a,b,s) read_write_mem(p,a,b,s,1)
...
LV2_HOOKED_FUNCTION_PRECALL_SUCCESS_8(int, load_process_hooked, (process_t process, int fd, char *path, int r6, uint64_t r7, uint64_t r8, uint64_t r9, uint64_t r10, uint64_t sp_70))
{
	...
	const uint8_t is_rebug = strstr(path, "RBGTLBOX2") != 0;
	if (!is_rebug && (strstr(path, "/dev_bdvd/PS3_GAME/USRDIR/") || strstr(path, "/dev_hdd0/game/")))
	{
...
int get_text_info(process_t process, const char* modname, uintptr_t *outBase, uint64_t *outSize)
{
#define MAX_PRX 256
	sys_prx_id_t list[MAX_PRX];
	uint32_t unk_list[MAX_PRX];
	memset(list, 0, sizeof(list));
	memset(unk_list, 0, sizeof(unk_list));
	uint32_t n, n2;
	...
	if (prx_get_module_list(process, list, unk_list, MAX_PRX, &n, &n2) == 0)
	{
		for (uint32_t i = 0; i < n; i++)
		{
			...
			if (prx_get_module_info(process, list[i], &modinfo, NULL, &text_segment) == 0)
			{
				...
				if (strstr(modinfo.name, modname) != 0)
				{
					*outBase = text_segment.base;
					*outSize = text_segment.memsz;
					return 1;
				}
			}
		}
...
// masked by ff
uintptr_t PatternScan(process_t process, const uintptr_t module_base, const uint64_t module_size, const void *signature, const uint64_t signature_size, const uint64_t offset)
{
...
LV2_HOOKED_FUNCTION_POSTCALL_3(int, load_module_hook, (char *path, uint64_t param_2, void *param_3))
{
	...
	if (proc_load_okay && strstr(path, "/dev_flash/sys/external/libsysmodule.sprx"))
	{
		...
		{
			uint32_t buf = 0;
			const uint32_t check = 0x7f454c46; // '\x7fELF'
			const int ret = read_mem(process, 0x00010000, &buf, sizeof(buf));
			DPRINTF2("read %x check %x ret %d\n", buf, check, ret);
			if (ret == 0)
			{
				if (buf == check)
				{
...
#define _countof(a) sizeof(a) / sizeof(*a)
								uint32_t patch_copy[_countof(liblv2_patch)];
#undef _countof
...
#define PRINT_BRANCH(b, s) // DPRINTF2(#b ": from 0x%lx 0x%x to 0x%lx\n", s, b, s + READ_JUMP_OFFSET(b))
...
{
	const uint64_t load = get_syscall_address(_SYS_PRX_LOAD_MODULE_ON_MEMCONTAINER);
